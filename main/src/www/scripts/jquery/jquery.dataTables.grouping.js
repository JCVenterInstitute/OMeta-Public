...
(function ($) {
    ...
    $.fn.rowGrouping = function (options) {
        ...
        function _fnOnGrouped() {
            /* hkim - added to collapse all rows after grouping is done */
        }
        ...
        return this.each(function (index, elem) {
            ...
            function _fnIsGroupCollapsed(sGroup) {        //fnCollapseGroup(sGroup);
            ...
            function fnExpandGroup(sGroup) {
                ...
                if (properties.oHideEffect != null)
                    ...
                    /* hkim - modified to expand only direct child group */
                    $(".group-item-" + sGroup, oTable).show();

                    if(sGroup.indexOf('-')<0)  {
                        $("tr[data-group^='" + sGroup+"']", oTable).hide();
                    }
...
//                          sGroupData = oSettings.aoData[oSettings.aiDisplay[iDisplayIndex]]._aData[oSettings.aoColumns[properties.iGroupingColumnIndex].mDataProp];
                        ...
                        if (bUseSecondaryGrouping) {
                            ...
                            /* hkim - two level collapsible table show second level groups when sorting or searching with first level group collapsed */
                            if (_fnIsGroupCollapsed(sGroupCleaned)) {
                                $(nGroup2).hide();
                            }
