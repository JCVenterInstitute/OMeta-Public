-- PROJECT
-- select * from  project where projet_is_public = 1;
SELECT
...
FROM project
WHERE projet_is_public = 1 and (date(projet_create_date)=CURRENT_DATE()-INTERVAL 1 DAY or date(projet_modified_date)=CURRENT_DATE()-INTERVAL 1 DAY or
  date(projet_create_date)=CURRENT_DATE() or date(projet_modified_date)=CURRENT_DATE())
; 
