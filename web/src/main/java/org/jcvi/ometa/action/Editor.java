    ...
    public String projectEditProcess() {
        ...
        try {
            if (editType != null) {
                ...
                if (readPersister.getLookupValue(LOOKUP_VALUE_NAME_PROJECT_UPDATE, Constants.EVENT_TYPE_LV_TYPE_NAME) == null)
                    lkvList.add(this.createLookup(LOOKUP_VALUE_NAME_PROJECT_UPDATE, Constants.EVENT_TYPE_LV_TYPE_NAME, Constants.STRING_DATA_TYPE));
                ...
                if (editType.equals("project")) {
                    ...
                    if (isPublic != project.getIsPublic() || isSecure != project.getIsSecure()) {
                        if (isPublic != project.getIsPublic()) {
                            if (readPersister.getLookupValue(LOOKUP_VALUE_NAME_IS_PUBLIC, Constants.ATTRIBUTE_LV_TYPE_NAME) == null) {
                                lkvList.add(this.createLookup(LOOKUP_VALUE_NAME_IS_PUBLIC, Constants.ATTRIBUTE_LV_TYPE_NAME, Constants.INT_DATA_TYPE));
                            }
                            ...
                            if (!existingEmaNames.contains(LOOKUP_VALUE_NAME_IS_PUBLIC))
                                emaList.add(
                                        this.createEventMetaAttribute(
                                                ...
                                                false, true, Constants.INT_DATA_TYPE, "Project availability", false
                                        )
                                );
                        ...
                        if (isSecure != project.getIsSecure()) {
                            if (readPersister.getLookupValue(LOOKUP_VALUE_NAME_IS_SECURE, Constants.ATTRIBUTE_LV_TYPE_NAME) == null) {
                                lkvList.add(this.createLookup(LOOKUP_VALUE_NAME_IS_SECURE, Constants.ATTRIBUTE_LV_TYPE_NAME, Constants.INT_DATA_TYPE));
                            }
                            ...
                            if (!existingEmaNames.contains(LOOKUP_VALUE_NAME_IS_SECURE))
                                emaList.add(
                                        this.createEventMetaAttribute(
                                                ...
                                                false, true, Constants.INT_DATA_TYPE, "Project security", false
                                        )
                                );
                ...
                if (editType.equals("sample")) {
                    ...
                    if (isPublic != sample.getIsPublic()) {
                        if (readPersister.getLookupValue(LOOKUP_VALUE_NAME_IS_PUBLIC, LOOKUP_VALUE_TYPE_ATTRIBUTE) == null) {
                            lkvList.add(this.createLookup(LOOKUP_VALUE_NAME_IS_PUBLIC, LOOKUP_VALUE_TYPE_ATTRIBUTE, Constants.INT_DATA_TYPE));
                        }
